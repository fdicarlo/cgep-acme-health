...
test_missing_logging_fails if {
	some msg in api_logging.deny with input as missing_logging
	contains(msg, "164.312(b)")
}

test_missing_stage_fails if {
	some msg in api_logging.deny with input as missing_stage
	contains(msg, "164.312(b)")
}

test_empty_logging_fails if {
	some msg in api_logging.deny with input as empty_logging
	contains(msg, "164.312(b)")
}
